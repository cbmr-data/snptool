...
# NOTE: The self.header part is not actually used. Header can be printed either with header=True or by printing self.header.

class SNPstreamer(SubprocessHandler):
    """An object for iterating over SNP-based input."""
    __name__ = "SNPstreamer"

    def __init__(self, filename, command='view', header=True, options="--output-type v", regions=[], *args, **kwargs):
        ...
        options = shlex.split(options) if header is True else shlex.split(options) + ["--no-header"]
        command = ['bcftools', command] + options
        command += ['--regions-file', '-', filename] if regions else [filename]
        super().__init__(command, input_data=regions, *args, **kwargs)
        self._process_header(filename)
